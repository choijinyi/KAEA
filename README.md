...
> 🎯 **이름 빙고 게임**이 `/bingo` 경로에 함께 배포됩니다. 두 가지 모드를 지원합니다.
>
> - **퀴즈 빙고** (`/bingo/host`, `/bingo/play`): 참가자가 이름 25개를 적고, 진행자가 정답
>   보드에 100·200·300·500점짜리 이름을 3개씩(총 12개) 적으면 일치한 이름에 줄이 그어지고
>   점수가 합산됩니다. 진행자 화면에 실시간 순위표가 표시됩니다.
> - **클래식 빙고** (`/bingo/classic/host`, `/bingo/classic/play`): 진행자가 이름 25개를
>   등록하면 참가자가 자신만의 5×5 빙고판을 만들고, 진행자가 이름을 지우면 판에서 자동으로
>   지워지며 가로·세로·대각선 빙고 줄 수가 표시됩니다.
>
> 동기화는 Supabase Realtime(+폴링 백업)으로 동작합니다.
